...
const MoviesContainerPage = () => {
  ...
  const { data: newMovies, isLoading: newMoviesLoading } = useGetNewMoviesQuery();
  ...
  if (newMoviesLoading || topMoviesLoading || genresLoading || randomMoviesLoading) {
    return <div className="text-center text-lg font-semibold mt-10">Loading movies...</div>;
  }
    ...
    <div className="flex flex-col lg:flex-row lg:justify-between items-center">
      ...
      <section className="flex flex-col justify-center items-center w-full lg:w-auto">
        <div className="w-full lg:w-[100rem] mb-8">
          ...
          {randomMovies?.length ? <SliderUtil data={randomMovies} /> : <p>No movies available.</p>}
        </div>

        <div className="w-full lg:w-[100rem] mb-8">
          <h1 className="mb-5">Top Movies</h1>
          {topMovies?.length ? <SliderUtil data={topMovies} /> : <p>No top movies available.</p>}
        </div>

        <div className="w-full lg:w-[100rem] mb-8">
          <h1 className="mb-5">Choose Movie</h1>
          {filteredMovies?.length ? <SliderUtil data={filteredMovies} /> : <p>No movies available.</p>}
        </div>
      </section>
    </div>
  );
